...
matplotlib.use('agg')
from abm.agent.dqn import DQNAgent
...
class Agent(pygame.sprite.Sprite):
    ...
    def __init__(self, id, radius, position, orientation, env_size, color, v_field_res, FOV, window_pad, pooling_time,
                 pooling_prob, consumption, vision_range, visual_exclusion, patchwise_exclusion=True, behave_params=None,epsilon=1.0, epsilon_decay=0.995, epsilon_min=0.01, gamma=0.99):
        """
        ...
        :param
        ption: (resource unit/time unit) consumption efficiency of agent
        ...
        self.input_soc_v_field = np.zeros((3,self.v_field_res))  # social visual projection field
        ...
        self.vis_counter = 0# np.zeros(self.v_field_res)  # visual projection field
        ...
        self.nn_social_v_field =np.zeros(self.v_field_res*3)

        self.policy_network = DQNAgent(state_size=self.v_field_res+ 1, action_size=3)

    def reset(self):
            """
            Reset relevant values of the agent after each training episode.
            """
    ...
    def update_decision_processes(self):
        ...
        dw = self.Eps_w * (np.mean(self.exploit_soc_v_field)) - self.g_w * (
    ...
    def update(self, agents):
        ...
        if self.type=="mechanistic":
            ...
            if self.policy_network.action==0:
                self.set_mode("explore")
            elif self.policy_network.action==1:
                self.set_mode("exploit")
            elif self.policy_network.action==2:
                self.set_mode("relocate")
            #self.set_mode("explore" if self.policy_network.action==0 )
    ...
    def calc_social_V_proj(self, agents):
        ...
        if self.visual_exclusion:
            self.exploit_soc_v_field= self.projection_field(exploit_agents, keep_distance_info=False,
                                                     non_expl_agents=non_exploit_agents)
            self.explore_soc_v_field= self.projection_field(explore_agents, keep_distance_info=False,
                                                     non_expl_agents=non_explore_agents)
            self.reloc_soc_v_field= self.projection_field(reloc_agents, keep_distance_info=False,
        ...
        #self.nn_social_v_field = np.concatenate((self.exploit_soc_v_field, self.reloc_soc_v_field,self.explore_soc_v_field))
        self.nn_social_v_field = self.exploit_soc_v_field
    ...
    def projection_field(self, obstacles, keep_distance_info=False, non_expl_agents=None, fov=None):
        ...
        num_categories = 3 # 0: expl. agent, 1: exploring, 2: relocating
        v_field = np.zeros((num_categories, self.v_field_res))  # Initialize 2D visual field
