...
from abm.simulation.sims import Simulation, Simulation_predprey
...
def start_pred_prey(parallel=False, headless=False):
    ...
    with ExitStack() if not headless else Xvfb(width=vscreen_width, height=vscreen_height) as xvfb:
        sim = Simulation_predprey(N=int(float(envconf["N"])),
