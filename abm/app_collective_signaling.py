...
def setup_environment():
    ...
    env_file_dir = root_abm_dir / "abm" / "projects" / "cooperative_signaling"  # Path(__file__).parent
...
def start(parallel=False, headless=False, agent_behave_param_list=None):
    ...
    with ExitStack() if not headless else Xvfb(width=vscreen_width, height=vscreen_height) as xvfb:
        sim = CSSimulation(N=int(float(envconf["N"])),
                           ...
                           # Collective Signaling Parameters
                           ...
                           probability_of_starting_signaling=float(envconf["SIGNALLING_PROB"]),
                           des_velocity_res=float(envconf["RES_VEL"]),
                           res_theta_abs=float(envconf["RES_THETA"]),
                           agent_signaling_rand_event_update=float(envconf["SIGNAL_PROB_UPDATE_FREQ"]),
                           )
