...
for wi in range(fig_shape[0]):
    for hi in range(fig_shape[1]):
        ...
        #     print(hi, wi, ni)
        plt.axes(ax[wi, hi])
        collapsed_data = np.load(os.path.join(data_path, f"coll_eff_N{Ns[wi]}_Rp{Rps[hi]}.npy"))
        ...
        agent_dim = 4
        if collapsed_data.shape != reference_shape:
            collapsed_data_raw = collapsed_data.copy()
            collapsed_data = np.zeros(reference_shape)
            collapsed_data[0:collapsed_data_raw.shape[0], 0:collapsed_data_raw.shape[1]] = collapsed_data_raw[0:reference_shape[0], 0:reference_shape[1]]
        im = plt.imshow(collapsed_data, vmin=min_data_val, vmax=max_data_val)
            ...
            # labels = np.loadtxt(os.path.join(data_path, f"coll_eff_N{Ns[ni]}_Rp{Rps[hi]}.txt"), dtype=str)
            #
