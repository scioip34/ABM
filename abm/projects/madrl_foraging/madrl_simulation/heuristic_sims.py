...
from abm.projects.madrl_foraging.madrl_agent.heuristic_agent import HeuristicAgent as Agent
...
class HeuristicSimulation(Simulation):
    def __init__(self, **kwargs):
        ...
        self.num_episodes = 1 #learning_params.num_episodes
    ...
    def assign_resources_to_agents(self):
        ...
        for ag in sorted(self.agents, key=lambda x: min(
            ...
            smallest_dist = float('inf')  # Use infinity as the initial smallest distance
    ...
    def step(self,turned_on_vfield):
        ...
        for ag in self.agents:
            ...
            ag.calc_resource_V_proj([ag.closest_patch])
    ...
    def initialize_environment(self):
        ...
        for ag in self.agents:
            ...
            if ag.env_status == 1:
                ...
                ag.soc_v_field.tolist() + [1.0]).unsqueeze(0).to(device)
                #else:
        ...
        while self.t < self.T:
            ...
            for ag in self.agents:
                ...
                if ag.env_status == 1:
                    ...
                    ag_resc_overlap = self.agent_resource_overlap([ag])
                    resc= list(ag_resc_overlap.keys())[0]
                    ag.next_state_tensor = torch.FloatTensor(ag.soc_v_field.tolist() + [1.0]).unsqueeze(0).to(device)

                else:

                    ag.next_state_tensor = torch.FloatTensor(ag.soc_v_field.tolist() + [0.0]).unsqueeze(0).to(device)


                ag.policy_network.state_tensor = ag.next_state_tensor
                ag.policy_network.last_action = ag.action_tensor.item()
